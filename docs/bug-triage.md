...
### BUG-12: term-average fetchers read `source_type`/`source_id` from `grades` instead of `assessments`, silently breaking gradebook-link dedup

**Found during:** 2026-08-07, Bucket 1 PR 4a (Student 360) live e2e run — `Student360Service`'s academic aggregation was the first real invocation of `fetchStudentTermAverageInputs` since it shipped in the Foundation PR (that PR's own description noted it had "no current apps/api caller" at the time), and the live e2e suite failed immediately with a Postgres `column grades.source_type does not exist` error.

**File:** `apps/api/src/attendance/fetchers.ts` (`fetchStudentTermAverageInputs`), `apps/web/lib/grading/fetchStudentTermAverage.ts` (its apps/web twin — both introduced in Bucket 1 PR 7, the shared attendance/grading helpers Foundation).

**What was wrong:** Both fetchers selected `source_type`/`source_id` directly off the top-level `grades` row. Those columns live on `assessments` (`supabase/migrations/20260728000081_assessments_source_link.sql`, Bucket 1 PR 2b) — `grades` has never had them. Every grade's `source_type` therefore silently resolved to its TypeScript-side fallback `'DIRECT'` and `source_id` to `null`, regardless of the assessment's real link state. This broke the dedup check `calculateStudentTermAverage()` depends on to skip an already-linked homework/quiz score when reconciling `homework_completions`/`quiz_attempts` against `grades`: a student with an already-linked homework or quiz had that same score counted **twice** toward their term average — once via its cascaded `grades` row, once again via the raw source table — inflating `overall_average_percentage` on both report-card renderers (`apps/web/app/report-card/[studentId]/page.tsx`, `apps/web/app/print/report-card/[studentId]/page.tsx`) and Student 360's academic section.

**Impact window:** `main` had this bug live continuously from the Foundation PR's merge, commit `1603abc` (2026-08-07T18:38:54+03:00), until B1-4a's merge, commit `5eee9f9` (2026-08-08T12:03:06+03:00) — **17h24m12s**. The fix itself (commit `70df9d4`, 2026-08-07T23:27:20+03:00) was authored on the B1-4a feature branch during that PR's own e2e verification, before the branch merged — it never existed on `main` as an independent hotfix; the fix only reached production when `5eee9f9` carried it in as part of the B1-4a merge.

**Report card audit trail:** investigated whether any report card generated during the window could be retroactively identified — **no audit trail exists anywhere in this codebase for report-card views, prints, or generations.** Confirmed via a full read of both report-card page components: zero write calls on render (all-read Supabase queries only), and `PrintButton` in both is a pure client-side `window.print()` with no network request at all — the server never learns a "print" happened. Confirmed via every `action` value ever inserted into `audit_logs` across `apps/api/src` (~70 distinct values in current use) — none reference report cards; the closest is `student.view_360`, an unrelated feature. Confirmed via `student_report_cards`' own schema — only sign-off workflow columns (`is_published`, `published_at`, `locked_at`, `signed_by_id`, `class_teacher_comment`, `head_teacher_comment`), no view/print/generation telemetry of any kind. Report cards are rendered live as HTML/React with no PDF-generation step and no Storage artifact, so there's no stored-file timestamp to check either. **Retroactively identifying which specific report cards were generated or printed during the 17h24m window is not possible from anything this application persists.** Pilot schools were not actively using the platform during this window (pre-launch/mid-development) — real-world impact is assessed as effectively zero, but this remains a live gap (no audit trail for report-card generation at all) worth a backlog item if report-card correctness ever becomes an audited/compliance concern.

**Fix (2026-08-07/08, Bucket 1 PR 4a, commit `70df9d4`):** both fetchers corrected to select `source_type`/`source_id` off the joined `assessment` embed (`assessment:assessments!inner(..., source_type, source_id, ...)`) instead of the top-level `grades` row.

**Verification:** the same live e2e path that caught the bug (`apps/api/test/student-360.e2e-spec.ts`) re-run green after the fix, plus the full existing unit suite (235/235 at the time) unaffected — no test had asserted the specific `source_type`/`source_id` join shape before this, since the mocked-Supabase unit suite has no way to validate real column existence in the first place.

**Lesson:** a mocked-Supabase unit test asserts the code calls `.select('source_type, source_id')` and returns whatever the mock is told to return — it structurally cannot catch that the *real* table doesn't have those columns. Only a live e2e run against the actual schema surfaces a schema-vs-code mismatch like this. This is the second bug in this project (after BUG-5's bulk-migration drift) caught specifically by live-database verification that a mocked-query test suite cannot catch by construction — worth keeping a live e2e run in the standard verification loop for any PR touching a shared-helper fetcher, not just for RLS/cross-tenant correctness.
