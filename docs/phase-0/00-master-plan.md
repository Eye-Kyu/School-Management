...
## Status: Formally closed 2026-07-29

Three of Phase 0's five sub-sprints shipped: **1a (SMS via Africa's Talking)**, **2 (M-Pesa Paybill reconciliation + PDF receipts)**, and **4 (NEMIS export, accessibility, hardening, pre-CBC cleanup)**. **Sub-sprint 1b (WhatsApp)** and **sub-sprint 3 (CBC compliance)** never unblocked during this sprint's window — Meta Business API approval and KICD competency-list/sample-report-card verification are both still pending, external, owner-initiated actions, not dev-time gaps. Both are relocated to [`docs/phase-1/00-backlog.md`](../phase-1/00-backlog.md) rather than left open-ended here.

Phase 0 is closed as of this date on the basis of the honest, per-item Definition of Done status below — not a claim that every line item shipped. See `docs/phase-0/phase-0-status-internal.md` for the full engineering write-up and `docs/phase-0/phase-0-status-public.md` for the pilot-school-facing summary.
...
- [~] A parent with SMS enabled receives a real SMS (attendance or fee-due, at minimum) via a live Africa's Talking account, for at least one pilot school. **Infrastructure shipped and e2e-tested in sub-sprint 1a; no live Africa's Talking account or pilot-school send has happened in this environment.** Owner action: provision a live Africa's Talking account and confirm one real send.
- [→ Phase 1] WhatsApp is either delivered the same way, or explicitly deferred with a documented reason (Meta approval pending) — not silently required. Deferred, reason documented. See [`docs/phase-1/00-backlog.md`](../phase-1/00-backlog.md).
- [~] A parent can pay via Paybill directly (not through the app) and see their child's fee balance update within a few minutes, with no admin action, for at least one real test transaction. **Infrastructure shipped and e2e-tested in sub-sprint 2; no live Daraja account or real pilot-school transaction has happened in this environment.** Owner action: confirm Daraja production credentials and one real transaction.
- [x] Every successful payment (app-initiated or Paybill) produces a downloadable PDF receipt. Shipped and e2e-tested, sub-sprint 2.
- [x] An admin can view and manually match at least one deliberately-unmatched test payment. Shipped and e2e-tested, sub-sprint 2.
- [→ Phase 1] A teacher can tag an assessment with a CBC competency and see an aggregated EE/ME/AE/BE level for at least one student. Deferred, reason documented. See [`docs/phase-1/00-backlog.md`](../phase-1/00-backlog.md).
- [→ Phase 1] An admin can generate CBC report cards for an entire class in one action, with AI-drafted, teacher-editable per-competency narratives. Deferred, reason documented. See [`docs/phase-1/00-backlog.md`](../phase-1/00-backlog.md).
- [x] NEMIS export produces a file containing all currently-required fields for an admin's own school's students. Shipped and e2e-tested, sub-sprint 4.
- [~] The 15 swept pages pass axe DevTools with zero critical/serious violations, or have an explicit, owner-approved "deferred, needs redesign" note. **Every statically-findable issue fixed via a repo-wide `eslint-plugin-jsx-a11y` ruleset and per-page fixes, sub-sprint 4 — but no axe DevTools/screen-reader run has happened, since this environment has no browser.** See [`docs/audits/accessibility-follow-ups.md`](../audits/accessibility-follow-ups.md).
- [x] `docs/phase-0/pentest-scope.md` exists and is ready to send to a candidate pentester. Verified current as of sub-sprint 4.
- [~] A public status page is live and reflects real API/DB/frontend availability. **`GET /health` now does a real DB check (sub-sprint 4) and the web app has status-page groundwork in place; the actual public status-page provider account (e.g. Better Stack) is still an owner action, not live.**
- [x] All new or changed backend logic has cross-tenant e2e test coverage, following this repo's established `cross-tenant.e2e-spec.ts` convention. Confirmed for every sub-sprint that shipped code this Phase 0 window (1a, 2, 4) plus the Phase 0 closure PR's own fixes (BUG-3, BUG-4).
- [x] `EXECUTION_PLAN.md` reflects all of the above as checked items. Updated in this Phase 0 closure PR.
...
- **`assessments` table schema drift**: the committed migration (`20260527000012_gradebook.sql`) does not match the live table at all — the real columns are `teacher_id NOT NULL, description, max_marks, assessment_date`, none of the migration's `kind`/`weight`/`max_score`/`date`/`created_by_id`. Should be reconciled (a corrective migration, or at minimum a documented note) independent of Phase 0, since it risks confusing any future work that trusts the migration history. **Resolved in sub-sprint 4, 2026-07-28** — turned out `packages/db/prisma/schema.prisma`'s `Assessment` model was already correct and already matched the live table; only the migration file itself was wrong, and it had never actually run (an earlier `prisma db push` created the real table first). A new idempotent corrective migration makes a *fresh* bootstrap produce the right shape too, without touching production (already correct) or editing the original migration file.
- **The two existing report-card renderers disagree** on both scoring math (total-marks-ratio vs. average-of-percentages) and letter-grade boundaries (75/60/50/40 vs. 80/70/60/50) for the same student/term. A real, pre-existing data-integrity/trust issue, independent of CBC — worth a dedicated bug-fix pass. **Resolved in sub-sprint 4, 2026-07-28** — project owner confirmed (via direct question, not assumed) the unweighted mean-of-percentages/80-70-60-50 scheme as authoritative; both renderers now import a shared `packages/types/src/grading.ts` instead of each computing it independently.
...
- **Accessibility has been a written-down, unenforced policy** (`EXECUTION_PLAN.md`'s own "always-on tracks": *"every new screen passes axe DevTools before merge"*) — worth the owner knowing this is a policy-vs-practice gap, not just a one-time sweep needed. **Partially resolved in sub-sprint 4, 2026-07-28** — `eslint-plugin-jsx-a11y`'s recommended ruleset is now explicit and repo-wide (previously only invisibly partial via Next's own bundling), and the 15 highest-traffic pages had every statically-findable issue fixed. **Still a policy-vs-practice gap in one real sense**: this environment has no browser/axe/screen-reader tooling, so "passes axe DevTools before merge" still can't be literally enforced by anyone working in this kind of environment — only a real browser session can close that gap. See `docs/audits/accessibility-follow-ups.md`.
- **`GET /health` is hardcoded `OK`** with no real DB check, unlike the rich SuperAdmin-only health endpoint. A status page watching a permanently-"OK" endpoint provides false assurance — sub-sprint 4's plan already folds in a minimal real check. **Resolved in sub-sprint 4, 2026-07-28** — now pings the DB and checks notifications-dispatch staleness, returns a real 503 on failure.
