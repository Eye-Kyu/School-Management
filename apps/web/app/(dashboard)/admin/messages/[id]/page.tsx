...
export default async function AdminThreadPage({ params }: { params: { id: string } }) {
  ...
  const left = parent ?? admin;
  const right = teacher ?? (parent ? admin : null);
    ...
    <div className="max-w-2xl space-y-4">
      ...
      <div className="space-y-3">
        {(messages ?? []).map((msg) => {
          ...
          const isLeft = msg.sender_id === left?.id;
          return (
            <div key={msg.id} className={`flex items-end gap-2 ${isLeft ? 'justify-start' : 'justify-end'}`}>
              {isLeft && (
              ...
              <div className="max-w-[75%]">
                ...
                <p className={`text-[10px] mt-0.5 text-slate-400 ${isLeft ? 'text-left' : 'text-right'}`}>
                  {sender?.full_name} ·{' '}
