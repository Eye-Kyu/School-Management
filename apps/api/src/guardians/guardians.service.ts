...
export class GuardiansService {
  ...
  async list(accessToken: string) {
    ...
    for (const row of data ?? []) {
      const parent = row.parent as unknown as { id: string; full_name: string; email: string | null; phone: string | null };
      ...
      const student = row.student as unknown as { admission_no: string; user: { full_name: string } };
